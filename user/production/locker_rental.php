						...
						<div class="x_content">
							<form id="locker-rental-form" method="POST" class="form-horizontal form-label-left">
								...
								<div class="form-group">
									...
									<div class="col-lg-12">
										<input type="hidden" id="locker-no" name="locker-no" value="" required />
										<h1 id="locker-no-show"></h1>
									</div>
								</div>
		...
		if (selectedDate && selectedTimeSlot) {
			$.ajax({
				...
				success: function(response) {
					...
					var lockerShow = $('#locker-no-show');
					lockerSelect.empty();
					if (response.status === 'success') {
						availableLockers = response.data;
						if (availableLockers.length > 0) {
							var randomIndex = Math.floor(Math.random() * availableLockers.length);
							var locker = availableLockers[randomIndex];

							lockerSelect.val(locker.id);
							lockerShow.html(locker.locker_name);
						} else {
							lockerSelect.val('');
							lockerShow.html('Sorry, No Lockers Available!');
						}
					} else {
						lockerSelect.val('');
						lockerShow.html('Sorry, No Lockers Available!');
